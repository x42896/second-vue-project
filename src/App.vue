...
<template>
  ...
  <!-- v-for цикл для перебора массива users -->
  <!-- Обязательно нужно :key записть для уникальных значений каждого элемента который будем перебирать -->
  <!-- То есть у каждого значения должен быть index и его нужно создать по этому пишем (el, index) -->
  <div v-for="(el, index) in users" :key="index" class="user">
    <h3>{{ el.name }}</h3>
    <p>{{ el.name }} – <b>{{ el.pass }}</b></p>
  </div>
</template>
